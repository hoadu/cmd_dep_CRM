...
def insert_a_client():
    ...
    if len(str(okpo)) != 10:
        ...
        wb.sheets['database'].range("F22").value = "ОКПО должен содержать 8 цифр! "
        return None
    ...
    try:
        ...
        # create a new client
            ...
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Создан клиент " + str(client_name)

    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
...
def insert_a_service():
    ...
    try:
        ...
        # create a new project
            ...
            wb.sheets['database'].range("F22").color = (146, 208, 80)
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Клиенту " + str(client) + ' добавлен продукт ' + str(product)
    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
        wb.sheets['database'].range("F22").color = (240, 100, 77)
...
def insert_a_contact():
    ...
    if name == None:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = "Имя контакта является обязательным для заполнения!"
        return None
    else:
        name += " "
    ...
    try:
        ...
        # create a new project
            ...
            wb.sheets['database'].range("F22").color = (146, 208, 80)
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Создан контакт " + str(name) + ' ' + str(surname) + ' ' + str(family)
    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
...
def insert_a_bounded_contact():
    ...
    try:
        ...
        # create a new project
            ...
            wb.sheets['database'].range("F22").color = (146, 208, 80)
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Клиенту " + str(client) + ' назначен контакт ' + str(contact)
    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
...
def insert_a_bounded_status():
    wb = xw.Book.caller()
    date_added = wb.sheets['database'].range("A27").value.strftime('%Y-%m-%d %H:%M')
    ...
    try:
        ...
        # create a new project
            ...
            wb.sheets['database'].range("F22").color = (146, 208, 80)
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Статус клиента " + str(client) + ' изменен на ' + str(status)
    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
...
def insert_a_request():
    ...
    try:
        ...
        # create a new project
            ...
            wb.sheets['database'].range("F22").color = (146, 208, 80)
            wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ": Создано обращение от филиала " + str(branch)
    except sqlite3.IntegrityError as e:
        wb.sheets['database'].range("F22").color = (240, 100, 77)
        wb.sheets['database'].range("F22").value = str(datetime.datetime.now()) + ': ' + str(e)
...
def count_requests():
    ...
    cursor = conn.cursor()
    sql = '''SELECT branches.name AS 'Филиал', COUNT(requests.id) AS 'Обращений за период' FROM requests
        JOIN branches on branches.id = requests.branch
        WHERE requests.date_added BETWEEN ? AND ?
        GROUP BY branches.name'''
    start_date = wb.sheets['branches_report'].range("A3").value.strftime('%Y-%m-%d')
    end_date = wb.sheets['branches_report'].range("C3").value.strftime('%Y-%m-%d')
...
def generate_branches_report():
    ...
    conn = create_connection(db_file, )
    cursor = conn.cursor()
    sql = '''SELECT branches.name AS 'Филиал', products.name AS 'Продукт', statuses.name AS 'Статус', COUNT(bounded_statuses.status) AS 'Количество',
        ...
        JOIN statuses on statuses.id = bounded_statuses.status AND bounded_statuses.date_added = (
    ...
    cols = [column[0] for column in query.description]
    data = pd.DataFrame(query.fetchall(), columns=cols)
    wb.sheets['branches_report'].range('F8').options(index=False).value = count_requests()
...
def get_all_clients():
    ...
    conn = create_connection(db_file, )
    cursor = conn.cursor()
    sql = '''SELECT DISTINCT clients.okpo AS 'ОКПО',  clients.name AS 'Компания', markets.name AS 'Рынок', statuses.name AS 'Текущий статус',
        bounded_statuses.date_added AS 'Статус действителен с',
        (contacts.name || contacts.family) AS 'Контактное лицо', contacts.phone AS 'Телефон', contacts.email AS 'Email'
        FROM clients
        JOIN markets on markets.id = clients.market
        JOIN bounded_statuses on bounded_statuses.client = clients.okpo AND bounded_statuses.date_added = (
